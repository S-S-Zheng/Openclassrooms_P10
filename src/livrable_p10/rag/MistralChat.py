...
try:
    from livrable_p10.rag.utils.config import (
        MISTRAL_API_KEY, MODEL_NAME, SEARCH_K,
        APP_TITLE, NAME
    )
    from livrable_p10.rag.utils.vector_store import VectorStoreManager
except ImportError as e:
    st.error(
        f"Erreur d'importation: {e}. Vérifiez la structure de vos dossiers et les fichiers"
        "dans 'utils'."
    )
    st.stop()
...
def get_vector_store_manager():
    ...
    try:
        ...
        if manager.index is None or not manager.document_chunks:
            ...
            st.warning(
                "Assurez-vous d'avoir exécuté 'python indexer.py' après avoir placé vos"
                "fichiers dans le dossier 'inputs'."
            )
...
def generer_reponse(prompt_messages: list[ChatMessage]) -> str:
    ...
    try:
        logging.info(
            f"Appel à l'API Mistral modèle '{model}' avec {len(prompt_messages)} message(s)."
        )
