...
def init_db(reset_tables=False, engine=base_engine):
    ...
    Note
    ----
        L'import des tables est nécessaire ici, même s'ils ne sont pas appelés explicitement,
        afin qu'ils soient enregistrés dans le registre 'Base.metadata' avant l'exécution
        de 'create_all'.
    """
    if reset_tables:
