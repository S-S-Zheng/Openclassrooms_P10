...
class NBAInputSchema(BaseModel):
    ...
    player_name: str = Field(..., alias="Player", description="Nom du joueur")
    team_abbr: str = Field(..., alias="Team", description="Équipe du joueur (code à 3 lettres)")
    age: int = Field(..., alias="Age", ge=18, le=50, description="Âge du joueur")
    gp: int = Field(
        ..., alias="GP", ge=0, le=100, description="Nombre de matchs joués (Games Played)"
    )
    win: int = Field(
        ...,
        alias="W",
    ...
    time_played: float = Field(
        ..., alias="Min", ge=0.0, le=60.0, description="Minutes moyennes jouées par match"
    )
    points: int = Field(
        ..., alias="PTS", ge=0, le=5000, description="Points marqués en moyenne par match"
    )
    fgm: int = Field(
        ..., alias="FGM", ge=0, le=2500, description="Tirs réussis par match (Field Goals Made)"
    )
    fga: int = Field(
        ..., alias="FGA", ge=0, le=2500, description="Tirs tentés par match (Field Goals Attempted)"
    )
    fg_pct: float = Field(
        ..., alias="FG%", ge=0.0, le=100.0, description="Pourcentage de réussite aux tirs"
    )
    fifteen: int = Field(
        ..., alias="15:00:00", ge=0, le=999, description="Minutes jouées après 15:00 de jeu"
    )
    three_p_tried: int = Field(
        ..., alias="3PA", ge=0, le=999, description="Tirs à 3 points tentés par match"
    )
    three_p_pct: float = Field(
        ..., alias="3P%", ge=0.0, le=100.0, description="Pourcentage de réussite à 3 points"
    )
    ftm: int = Field(
        ..., alias="FTM", ge=0, le=999, description="Lancers francs réussis (Free Throws Made)"
    )
    fta: int = Field(..., alias="FTA", ge=0, le=999, description="Lancers francs tentés")
    ft_pct: float = Field(
        alias="FT%", ge=0.0, le=100.0, description="Pourcentage de réussite aux lancers francs"
    )
    oreb: int = Field(alias="OREB", ge=0, le=999, description="Rebonds offensifs")
    dreb: int = Field(alias="DREB", ge=0, le=999, description="Rebonds défensifs")
    reb: int = Field(alias="REB", ge=0, le=2000, description="Rebonds totaux")
    assists: int = Field(alias="AST", ge=0, le=999, description="Passes décisives (Assists)")
    turnovers: int = Field(alias="TOV", ge=0, le=999, description="Balles perdues (Turnovers)")
    steal: int = Field(alias="STL", ge=0, le=999, description="Interceptions (Steals)")
    blocks: int = Field(alias="BLK", ge=0, le=999, description="Contres (Blocks)")
    faults: int = Field(alias="PF", ge=0, le=999, description="Fautes personnelles")
    fantasy_points: int = Field(alias="FP", ge=0, le=9999, description="Fantasy Points")
    doubles: int = Field(
        alias="DD2",
        ge=0,
        le=99,
    ...
    net_rate: float = Field(
        alias="NETRTG", ge=-99.0, le=99.0, description="Net Rating = OFFRTG - DEFRTG"
    )
    ...
    assists_turnovers_rate: float = Field(
        alias="AST/TO", ge=0.0, le=99.0, description="Ratio passes / pertes de balle"
    )
    assists_rate: float = Field(
        alias="AST RATIO", ge=0.0, le=100.0, description="Ratio d’assists pour 100 possessions"
    )
    ...
    efg_pct: float = Field(
        alias="EFG%", ge=0.0, le=125.0, description="Effective Field Goal % (pondère les 3 points)"
    )
    ts_pct: float = Field(
        alias="TS%",
        ge=0.0,
    ...
    pace: float = Field(
        alias="PACE", ge=0.0, le=999.0, description="Rythme de jeu (possessions par 48 minutes))"
    )
    pie: float = Field(
        ...,
        alias="PIE",
    ...
    poss: int = Field(
        ..., alias="POSS", ge=0, le=9999, description="Nombre total de possessions jouées"
    )
...
class TeamInputSchema(BaseModel):
    ...
    team_abbr: str = Field(..., alias="Code", description="Équipe du joueur (code à 3 lettres)")
    full_name: str = Field(
        ..., alias="Nom complet de l'équipe", description="Nom complet de l'équipe"
    )
...
class SQLOutputSchema(BaseModel):
    ...
    # Identifiants
    name: Optional[str] = Field(None, description="Nom du joueur (issu de la table players)")
    team_abbr: Optional[str] = Field(
        None, min_length=3, max_length=3, description="Code équipe a 3 lettres"
    )
    full_name: Optional[str] = Field(None, description="Nom complet de l'équipe")
